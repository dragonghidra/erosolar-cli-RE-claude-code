# Angular SSR Erosolar mirror

> Commands in this document assume you are inside the `erosolar-angular/` folder at the repo root.

This app mirrors the dual-profile feed that the Erosolar CLI (general + Erosolar Code) prints inside the terminal. Angular now runs in SSR mode so the Express host can keep an in-process session open with whatever agent backend you point it at (local CLI, mock data, or a remote Cloud Run instance).
...
Need a live feed from your real workspace? This package already links the sibling `erosolar-ui-cli` module, so you can spin up the CLI bridge directly:

```bash
npm run ui:cli -- diag
```

From the repo root you can also run `./scripts/full-stack.sh` to automate the installation + build + SSR launch process. The script wires `erosolar-ui` into the backend using `AGENT_SOURCE=local-cli`, so the dashboard is immediately connected to a live Erosolar session without any manual environment juggling.

The `ui:cli` script behaves like running `erosolar-ui` globally—you can forward any Erosolar CLI arguments after the `--`.
...
1. **Terminal mirror** – the hero grid mirrors the Erosolar CLI (general + Erosolar Code) (mock data by default or the real CLI when you flip `AGENT_SOURCE=local-cli`).
...
2. Bootstrap the Firestore `projects/<id>` document and default connector catalog so security rules have something to validate:
   ...
   ```
   The script lives in `functions/scripts/bootstrap-project.mjs` and seeds a mock gateway, the local Erosolar CLI bridge, and a `firebase-cloud-agent` connector that knows how to deploy Hosting + Functions.
...
- Open the “Session settings” drawer (top of the hero) to manage session id, passphrase, and Firebase tokens. If you expose a Firebase config via `window.EROSOLAR_FIREBASE_CONFIG` the drawer enables a Google sign-in button; the resulting ID token is forwarded to the backend and stored securely via cookies.
...
If you prefer Angular's hot-module reloading, keep the erosolar-ui backend running (for example `npm run dev:ssr` or any other process that exposes the `/api/session` endpoints) and start `ng serve` in a second terminal:

```bash
# optional: override the backend URL (defaults to http://localhost:4000)
export EROSOLAR_UI_BACKEND_URL=http://localhost:4000
npm start
```
...
- `src/backend/gateways/*` exposes pluggable connectors:
  ...
  - `LocalCliGateway` spawns your local `erosolar --profile <name> --json …` (or any CLI that emits line-delimited JSON) and forwards stdout/stderr into the session.
  - `RemoteAgentGateway` polls a remote HTTP endpoint (for Cloud Run or another SSR host) so you can daisy-chain Erosolar mirrors.
...
| Mirror file   | `AGENT_SOURCE=mirror-file`          | Tails the `.jsonl` mirror written by `erosolar-ui` / Erosolar CLI for a read-only replay. |
...
LOCAL_CLI_COMMAND="erosolar --profile erosolar-code --json" \
LOCAL_CLI_CWD=$PWD \
npm run serve:ssr
```

Because the repo already links `erosolar-ui`, you can also drive the CLI through the helper script and keep arguments scoped to this workspace:

```bash
AGENT_SOURCE=local-cli \
LOCAL_CLI_COMMAND="npm run ui:cli -- erosolar --profile general --json" \
...
The gateway writes whatever you type into the browser input back to the CLI process via stdin, so you can keep any Erosolar CLI profile running headless and steer it from the mirror.

### Remote mirrors / Cloud Run

```
AGENT_SOURCE=remote-cloud \
REMOTE_AGENT_URL="https://erosolar-cloud-run.example.com" \
REMOTE_AGENT_TOKEN="bearer token if needed" \
npm run serve:ssr
```

The remote instance just needs to expose the same `/api/session` + `/api/session/stream` contract, so you can chain SSR pods or attach to `erosolar share --live` output.

### Mirror an existing Erosolar CLI session

Launch Erosolar CLI through `erosolar-ui` (or any process that writes the JSONL mirror) and point the Angular server at the file it prints in the banner:

```
AGENT_SOURCE=mirror-file \
EROSOLAR_UI_MIRROR_FILE=/Users/bo/GitHub/workspace/.erosolar-ui/mirror/erosolar-2025-11-16T17-20-58-852Z.jsonl \
npm run dev:ssr
```

The backend tails the log, emits every stdout/stderr line as chat messages, and surfaces process lifecycle events in the ops panel. Set `EROSOLAR_UI_SESSION_ID` (or `SESSION_ID`) if you want the dashboard to label the session differently from the filename. If you omit `EROSOLAR_UI_MIRROR_FILE` the server now looks for the newest `.jsonl` inside `<workspace>/.erosolar-ui/mirror/`, where the workspace is derived from `EROSOLAR_UI_WORKSPACE`, `WORKSPACE`, or the current working directory. That means you can simply run `erosolar-ui` in the same repo and start the Angular server without any extra configuration—the freshest mirror file will be picked up automatically.
...
- Signals keep the DOM in sync as SSE events arrive, so the UI behaves like the tmux feed you see in Erosolar CLI.
