...
This guide links the Erosolar Angular dashboard, the Firebase-backed persistence model, and the Erosolar CLI runtime so a background cloud agent can execute real work (tests, builds, deployments) while the UI streams live Firestore updates.

## Prerequisites

- Firebase project `erosolar-agents` with Hosting, Firestore, Functions, and Cloud Run APIs enabled.
- Local Firebase CLI + Admin credentials (service account JSON exported with `roles/editor` + `roles/firebase.admin`).
- Node.js 20+, npm, and the repo root checked out so both the CLI (`dist/bin/erosolar.js`) and the Angular workspace share the same files.

## 1. Configure Angular + Firebase

1. Populate both environment files with the production Firebase config (`src/environments/environment*.ts`). The Angular build now embeds the `erosolar-agents` API key so AngularFire can authenticate.
...
- Default connectors under `projects/<id>/connectors`:
  ...
  - `local-cli` to mirror a real Erosolar CLI session via `npm run ui:cli -- erosolar --profile erosolar-code`.
...
| --- | --- | --- |
| `repo_health_audit` | Manual or scheduled via Cloud Tasks | Launches the Erosolar CLI through the `local-cli` connector, runs `npm run lint`, `npm test`, and emits coverage/artifact summaries into `projects/<id>/agentRuns/<runId>/events`. |
| `deploy_hosting_bundle` | Manual from Run Board (`createAgentRun`) or chained after a successful audit | Runs `ng build --configuration production` from `erosolar-angular`, uploads the SSR bundle to Firebase Hosting target `app`, and logs the deployment version + Hosting channel URL. |
| `functions_release` | Manual or after merges that touch `functions/src/**` | Executes `npm run build` inside `erosolar-angular/functions` and calls `firebase deploy --only functions` so callable APIs (`createAgentRun`, `issueCommand`, `listConnectors`, demo executor) stay in sync with TypeScript sources. |
...
- After each deployment, run `npm start` in `erosolar-angular` to verify the Run Board lists `projects/<id>/agentRuns` and the connector gallery surfaces the newly seeded records.
...
This playbook keeps the repo, Firebase security model, and the Erosolar runtime aligned so the “cloud agent” can perform useful work—running audits, shipping SSR bundles, and releasing Functions—while the Angular dashboard mirrors every step in real time.
