...
- **SavedQueriesComponent**: lets users save queries (e.g., “Show Erosolar runs hitting MCP:github”). Each saved query stores Firestore composite-index metadata so Cloud Functions can hydrate it quickly.
...
- See `firebase/firestore.rules` and `firebase/firestore.indexes.json` for the exact implementation deployed to the `erosolar-agents` project.

## Deployment on Firebase
- **Hosting**: Build the Angular SSR bundle (`ng run erosolar-angular:serve-ssr`) and deploy via `firebase deploy --only hosting`. Configure rewrites to route `/api/**` to the Functions region so SSR + APIs share cookies.
