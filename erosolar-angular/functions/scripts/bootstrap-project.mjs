...
function defaultConnectors(projectId) {
  return [
    ...
    {
      id: 'local-cli',
      name: 'Local Erosolar CLI Bridge',
      kind: 'local-cli',
      description: 'Spawns the Erosolar CLI from this repo so you can mirror real workstation sessions into Firestore.',
      status: 'ready',
      metadata: {
        command: 'npm run ui:cli -- erosolar --profile erosolar-code',
        ...
        supportedPlaybooks: [
          {
            ...
            description: 'Runs lint + unit tests via the Erosolar CLI, writes summaries to projects/<id>/agentRuns events.',
          ...
          },
          {
            ...
            actions: ['npm run build --workspace erosolar-angular', 'firebase deploy --only hosting:app']
          },
          {
            id: 'functions_release',
            description: 'Compiles /functions (Node 22) and deploys callable APIs.',
            actions: ['cd erosolar-angular/functions', 'npm run build', 'firebase deploy --only functions']
          }
        ],
        observability: {
          runEvents: true,
