...
declare global {
  interface Window {
    EROSOLAR_FIREBASE_CONFIG?: FirebaseOptions;
  }
}
...
export class FirebaseLoginService {
  ...
  async signInWithPopup(): Promise<string | null> {
    ...
    let app;
    const existing = getApps().find((candidate) => candidate.name === 'erosolar-angular');
    if (existing) {
      app = existing;
    } else {
      app = initializeApp(config, 'erosolar-angular');
    }
  ...
  private readConfig(): FirebaseOptions | null {
    ...
    return window.EROSOLAR_FIREBASE_CONFIG ?? null;
  }
}
