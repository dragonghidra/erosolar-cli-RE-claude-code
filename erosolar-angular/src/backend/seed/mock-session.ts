...
export const mockChatMessages: ChatMessage[] = [
  {
    ...
    command: 'erosolar --profile erosolar-code --json stream',
    body: [
      'request: replace the placeholder marketing layout with the actual CLI chat feed.',
      'the browser view should feel identical to an Erosolar CLI session (general + erosolar-code).',
  ...
  {
    id: 'erosolar-code-diff',
    agent: 'erosolar-code',
    timestamp: '12:40:13',
    title: 'erosolar code · patch builder',
    caption: 'src/app/app.*',
    body: [
      'patched component data model into chatMessages + meters.',
      'rewired template so both erosolar profiles render like the terminal.',
  ...
  },
  {
    ...
    body: [
      ...
      '- ready to share using erosolar share --live.'
    ],
    status: 'tests pass',
    tokens: 'signal 98%',
    footer: 'sync contexts to keep erosolar general + code aligned.'
  }
];

export const mockStreamMeters: StreamMeter[] = [
  {
    label: 'Erosolar (general)',
    value: 'streaming',
...
export const mockShortcuts: Shortcut[] = [
  { keys: 'Shift+Enter', description: 'Send to Erosolar CLI' },
  { keys: 'Cmd+.', description: 'Interrupt streaming response' },
  { keys: 'Ctrl+K', description: 'Merge Erosolar general + code buffers' },
