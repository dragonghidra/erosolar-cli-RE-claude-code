...
export class MirrorFileGateway implements AgentGateway {
  ...
  readonly label = 'erosolar mirror log tail';

  private readonly shortcuts: Shortcut[] = [
    { keys: 'Shift+Enter', description: 'Send immediately to Erosolar CLI' },
  ...
  constructor(private readonly config: MirrorFileConfig) {
    ...
    this.status = {
      label: 'waiting for Erosolar mirror',
  ...
  private handleStartEntry(entry: MirrorEntry, emit: boolean, handler?: Handler): void {
    ...
    const argv = Array.isArray(data['argv']) ? data['argv'].map(String).join(' ') : 'erosolar';
    ...
    this.status = {
      label: 'erosolar session connected',
      detail: argv,
      tone: 'success',
    };
  ...
  private composeMessage(stream: MirrorStream, line: string, at?: string): ChatMessage {
    return {
      ...
      title: stream === 'stdout' ? 'erosolar cli · stdout' : 'erosolar cli · stderr',
