...
export class MockGateway implements AgentGateway {
  ...
  readonly label = 'sample erosolar mirror';
  ...
  private composeStreamingMessage(): ChatMessage {
    ...
    return {
      id: randomUUID(),
      agent: Math.random() > 0.5 ? 'erosolar-code' : 'erosolar',
      timestamp: now.toTimeString().split(' ')[0],
      title: Math.random() > 0.5 ? 'erosolar code · live' : 'erosolar (general) · watch',
      ...
      body: [
        ...
        'swap AGENT_SOURCE=local-cli to mirror the actual Erosolar CLI process.'
      ]
    };
  }
