...
const MIRROR_SUBDIR = ['.erosolar-ui', 'mirror'];
...
export const loadRuntimeConfig = (): RuntimeConfig => {
  ...
  if (source === 'mirror-file') {
    ...
    if (!file) {
      throw new Error(
        'Set EROSOLAR_UI_MIRROR_FILE (or MIRROR_FILE) or run erosolar-ui so we can find a mirror under <workspace>/.erosolar-ui/mirror/*.jsonl.',
      );
    }

    const sessionId = deriveMirrorSessionId(file, process.env['EROSOLAR_UI_SESSION_ID'] ?? process.env['SESSION_ID']);
    return {
      source,
      sessionId,
      mirrorFile: {
  ...
  if (source === 'local-cli') {
    const expectJson = parseBoolean(process.env['LOCAL_CLI_JSON'], true);
      return {
        ...
        sessionId,
        localCli: {
          command: process.env['LOCAL_CLI_COMMAND'] ?? 'erosolar --profile erosolar-code --json',
